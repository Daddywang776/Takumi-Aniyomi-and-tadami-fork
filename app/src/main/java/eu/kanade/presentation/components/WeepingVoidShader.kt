...
half4 main(float2 fragCoord){
    ...
    if (uv.y < 0.02 && abs(uv.x) < w * 1.4) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (progress < 0.20) {
                ...
                dropY = startY - pStretch * 0.02;
                trailTop = startY;
                trailFade = 1.0;
            } else {
                // Phase 2: Detached and slowly drifting down in zero gravity
                float pFall = (progress - 0.20) / 0.80;
                dropY = startY - 0.02 - pFall * 0.48;    // drifts slowly down off the screen
            ...
            float bead = sin(uv.y * 150.0 - T * 12.0);
            ...
            if (dDrop < 0.015 && dropActive > 0.5) {
                float2 n2 = diff / 0.015;
                float3 normal = normalize(float3(n2.x, n2.y, sqrt(max(0.0, 1.0 - dot(n2, n2)))));
            ...
            if (trail > 0.01 && dropActive > 0.5) {
                ...
                tearSpecular = max(tearSpecular, trailSpec * verticalMask * trailFade);
            }
        }
    }
    ...
    if (u_light < 0.5) {
        ...
        // Tears are barely visible (almost transparent) by multiplying with 0.16
        float tears = clamp(tearField * TEAR_AMT, 0.0, 1.0) * 0.16;
        float3 tearCol = mix(float3(0.55, 0.0, 0.03), float3(0.95, 0.15, 0.08), tears);
        col = mix(col, tearCol, tears);
        col += float3(1.0, 0.6, 0.5) * tearSpecular * WET * 0.30;
        ...
        // Tears are barely visible (almost transparent) by multiplying with 0.14
        float tears = clamp(tearField * TEAR_AMT, 0.0, 1.0) * 0.14;
        float3 tearCol = mix(float3(0.5, 0.0, 0.03), float3(0.35, 0.0, 0.02), tears);
        col = mix(col, tearCol, tears);
        col += float3(0.95, 0.5, 0.45) * tearSpecular * WET * 0.20;
