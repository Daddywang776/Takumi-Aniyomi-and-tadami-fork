        ...
        Box(
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                downloadList.forEach { header ->
                    ...
                    items(header.subItems, key = { (it as MangaDownloadItem).download.chapter.id }) { subItem ->
                        val item = subItem as MangaDownloadItem
...
private fun reorderWithinHeader(
    downloadList: List<MangaDownloadHeaderItem>,
    targetHeader: MangaDownloadHeaderItem,
    targetDownload: eu.kanade.tachiyomi.data.download.manga.model.MangaDownload,
    toTop: Boolean,
): List<eu.kanade.tachiyomi.data.download.manga.model.MangaDownload> {
    val reordered = mutableListOf<eu.kanade.tachiyomi.data.download.manga.model.MangaDownload>()
    downloadList.forEach { header ->
        val downloads = header.subItems.map { (it as MangaDownloadItem).download }.toMutableList()
